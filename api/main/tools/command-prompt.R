...
eval_auroc(scca_full_chem_bio_result$pred_score, bio_cleaned, colors[8], T, F)
eval_auroc(scca_full_pheno_bio_result$pred_score, bio_cleaned, colors[7], T, F, T)
eval_auroc(scca_antidp_chem_bio_result, antidp_bio, colors[6], T, F, T)
eval_auroc(scca_antidp_pheno_bio_result, antidp_bio, colors[5], T, F, T)
legend("bottomright", c("chem predict bio (full)", "pheno predict bio (full)", "chem predict bio (antidepressant)", "pheno predict bio (antidepressant)"), cex = 0.7, lwd = c(1, 1), col = colors[8:5], inset = 0.05)
mtext("SCCA", line = 0.5)
