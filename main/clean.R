### [Cleaned data Info. - all compounds are joined by PubChem CID]
...
if(!exists("bio_cleaned")) {
  if(file.exists("../data/bio-cleaned.csv"))
    ...
    # remove compounds
    bio_cleaned <- bio[-removed_index,]
    # remove features
    bio_cleaned <- bio_cleaned[, colSums(bio) != 0]
...
if(!exists("chem_cleaned")) {
  if(file.exists("../data/chem-cleaned.csv"))
    ...
    removed_index <- which(rowSums(bio) == 0)
    # remove compounds
    chem_cleaned <- chem[-removed_index,]
    # remove features
    chem_cleaned <- chem_cleaned[, colSums(chem) != 0]
...
if(!exists("pheno_cleaned")) {
  if(file.exists("../data/pheno-cleaned.csv"))
    ...
    removed_index <- which(rowSums(bio) == 0)
    # remove compounds
    pheno_cleaned <- pheno[-removed_index,]
    # remove features
    pheno_cleaned <- pheno_cleaned[, colSums(pheno) != 0]
    write.csv(pheno_cleaned, "../data/pheno-cleaned.csv")
  }
}

### Separate antidepressant from data set (one time process)
# known antidepressants' PubChem CIDs crawled by python crawler
